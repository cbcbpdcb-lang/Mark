...
// 事实底账用品牌 A 的（PRESETS.direct_a），不改浏览器端代码。检索召回、空白题、编造和拒答在这里按 gold_sources.json（实验计划附录 D）判。
//
...
import { autoAttribution, expectsRefusal, hasRetrieval, isBlank, isRefusal, kbOf, recallHit, topK, unsupportedSpecifics, visibleGold } from './rag_lib.mjs';
...
const rows = ok.map((r, i) => {
  ...
  const kb = kbOf(gold, r.app);
  const specs = visibleGold(entry, kb);
  const blank = isBlank(entry);
  const shouldRefuse = expectsRefusal(entry, kb);
  const hit = recallHit(r, specs, K);
  ...
  // 允许“只给有来源的事实”的题：没拒答，但检索段落里都找得到、也没说错，同样算恰当
  const properRefusal = shouldRefuse && (refusal || (!!entry.allow_sourced && fab?.length === 0 && !v.wrong));
  ...
  if (shouldRefuse && !properRefusal) problems.push(['该拒答没拒答', kb === null ? '所有知识库都没有这道题的标准段落' : '这个应用的知识库里没有这道题的标准段落']);
  else if (blank && !shouldRefuse && !grounded) problems.push(['空白题没有依据回答', refusal ? '检索到了标准段落却拒答' : hit === false ? `前 ${K} 段没召回标准段落` : hit === null ? '没有检索记录' : '回答里有检索段落找不到的具体事实']);
  else if (!blank && !shouldRefuse && !v.wrong && !v.pending && !v.verifiable) problems.push(['没说对官方口径', v.mentioned ? '' : '回答里没点品牌名，产品规则按“没有提到”算']);
  return { r, v, entry, blank, shouldRefuse, hit, judgeable, fab, refusal, grounded, properRefusal, keyRight, problems };
});
...
const summary = present.map(app => {
  ...
  const refuseRs = rs.filter(x => x.shouldRefuse);
  ...
  const s = {
    ...
    blankN: blankRs.length, grounded: cnt(x => x.grounded), refuseN: refuseRs.length, refused: cnt(x => x.properRefusal),
  ...
  s.rates = {
    ...
    空白题有依据回答率: rate(s.grounded, s.blankN), 编造率: rate(s.fab, s.fabN), 恰当拒答率: rate(s.refused, s.refuseN),
...
const f1 = write('summary.csv', [
  ...
  '空白题回答', '空白题有依据', '空白题有依据回答率 %', '应拒答的回答', '恰当拒答', '恰当拒答率 %',
  ...
  s.blankN, s.grounded, p(s.grounded, s.blankN), s.refuseN, s.refused, p(s.refused, s.refuseN),
...
const failures = rows.filter(x => x.problems.length).map(x => {
  ...
  const hint = x.hit !== null ? ''
    : x.shouldRefuse ? '知识库里没有这道题的标准段落：可能是口径缺失，请人工判断'
    : x.judgeable ? '' : '这条回答没有检索记录，无法预判';
  return [
    x.r.app, x.r.query_id, qText.get(x.r.query_id) || '', x.r.repeat, x.problems.map(y => y[0]).join('、'),
    [(x.entry.gold || []).filter(g => typeof g === 'string').join(' '), x.entry.expect].filter(Boolean).join('：'),
...
const f2 = write('failures.csv', [
  '应用', '问题编号', '问题', '第几次', '问题类型', '标准段落和期望', '详情', `前 ${K} 段召回标准段落`,
...
console.log(`标准段落：${show(args.gold)}（${gold.version || '未标版本'}）`);
console.log('应用   有效  说错  待确认  关键事实  空白题有依据  恰当拒答  编造  召回');
for (const s of summary) {
  const f = (k, n) => (n ? `${k}/${n}` : '-');
  console.log(`${s.app.padEnd(5)} ${String(s.n).padStart(4)} ${String(s.wrong).padStart(5)} ${String(s.pending).padStart(7)} ${f(s.key, s.n).padStart(9)} ${f(s.grounded, s.blankN).padStart(13)} ${f(s.refused, s.refuseN).padStart(9)} ${f(s.fab, s.fabN).padStart(5)} ${f(s.rec, s.recN).padStart(5)}`);
}
